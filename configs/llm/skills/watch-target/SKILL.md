...
同一个 watcher pane 的同一类 wakeup 是互斥的：新的 `timer` 会替换旧 `timer`，新的 `ai-idle` 会替换旧 `ai-idle`，新的 `ai-running` 会替换旧 `ai-running`。不同 mode 可以并存，所以 auto-switch 可以同时给当前目标挂 `ai-running`、给更优主线挂 `ai-idle`，二者不会互相清掉。如果多个同 mode AI window 任意一个满足条件都应该唤醒，用同一次 `schedule-wakeup.sh` 调用重复传 `--target`，不要拆成多个同 mode wakeup。不要绕过 `schedule-wakeup.sh` 手写 `sleep` / 条件循环，否则多个 wakeup 可能互相交替提交消息，造成死锁或重复调度。
...
安排新 wakeup 前不需要手动检查旧 timer/condition watcher；统一交给 `schedule-wakeup.sh` 清理同一 watcher pane、同一 mode 的旧 wakeup。不要清理其它 watcher pane 或其它 mode 的 sleep/monitor 进程。
