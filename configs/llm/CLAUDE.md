...
- **orchestrator TMA Agent**：调用 skill 的当前 Agent，通常运行在当前 tmux session 的 `orchestrator` window 中，负责创建、分派、调度、观察和汇总其它 TMA Agent。
  - 一般调用TMA skill的是orchestrator TMA Agent，如果window名字不是orchestrator，自己修改一下。
- **worker / analysis TMA Agent**：由 orchestrator 创建的协作 Agent，运行在同一个 tmux session 的独立 window 中，负责一个明确的子任务。

#### 创建 TMA Agent

新建的 TMA Agent 默认应该是交互式的 Codex / Claude Code 会话，方便用户随时切到对应 window 继续协作。除非任务特殊指明，否则默认使用 tmux 全局环境变量 `TMUX_AI_TOOL` 指定的 AI Agent：

```bash
AI_TOOL="$(tmux show-environment -g TMUX_AI_TOOL | sed 's/^TMUX_AI_TOOL=//')"
case "$AI_TOOL" in
  codex)  TOOL_CMD="codexr" ;;
  claude) TOOL_CMD="clauder" ;;
  *) echo "Unsupported TMUX_AI_TOOL: $AI_TOOL" >&2; exit 1 ;;
esac
```
...
tmux send-keys -t "$TARGET" "$TOOL_CMD" C-m
```
...
### 注意事项
一般在 tmux capture-pane 时，会看到最近一次输入提示的内容，其中可能是包含 TUI 的提示信息而非用户真实输入，不要被误导。
